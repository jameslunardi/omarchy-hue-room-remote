...
- **`cleanup.sh`** (`test_cleanup_sh.sh`): `secure_remove`, extracted the
  ...
  target, a missing path is a silent no-op, and a real directory at the
  target path is left alone instead of aborting the caller (run inside
  its own `set -e` subshell, since this test file's own options are
  laxer and wouldn't otherwise exercise that failure mode).
